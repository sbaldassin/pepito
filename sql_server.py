...
from tests.db.repositories.q_net_dw_fact_free_spin_repository import QNetDwFactFreeSpin
...
def get_freespin_by_customer_id():
    customer_id = request.args.get("customer_id")
    merchant_id = request.args.get("merchant_id", int(get_config().get("api", "merchant_id")))
    freespin = QNetDwFactFreeSpin().get_by_external_customer_id(customer_id, merchant_id)
